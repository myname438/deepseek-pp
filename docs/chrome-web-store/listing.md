...
- Upload package: `dist/deepseek-plus-plus-0.4.2-chrome.zip`
...
DeepSeek++ enhances the DeepSeek web app with persistent memory, reusable skills, system prompt presets, MCP tool execution, and scheduled automation.

Use it to keep useful facts across conversations, trigger custom skills with slash commands, connect user-configured MCP tools, and let DeepSeek continue a task after tool results are returned. The extension runs on chat.deepseek.com and provides a side panel for managing memories, skills, presets, MCP servers, automation tasks, sync settings, and visual preferences.
...
- MCP tool management for Streamable HTTP, SSE, local bridge, and native messaging transports.
...
DeepSeek++ 为 DeepSeek 网页版增加长期记忆、Skill 技能、系统提示词预设、MCP 工具调用和自动化任务。

你可以用它跨对话保存有用信息，通过 `/skill` 快速切换工作模式，连接自己配置的 MCP 工具，并让 DeepSeek 在工具结果返回后继续推进任务。扩展只在 chat.deepseek.com 运行，侧边栏用于管理记忆、技能、预设、MCP 服务、自动化任务、同步设置和个性化选项。

核心功能：

- 长期记忆：保存用户画像、行为反馈、话题上下文和参考资料。
- Skill 系统：支持斜杠命令触发内置或自定义技能。
- MCP 工具：支持 Streamable HTTP、SSE、本地 bridge 和 Native Messaging。
