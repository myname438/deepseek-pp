...
export const zhCN = {
  ...
  sidepanel: {
    ...
    projectsPage: {
      title: '项目',
      description: '把本地文件或 GitHub 仓库作为项目上下文附加到提示词中，每次提问会自动选择相关片段。',
      summary: '{projects} 个项目，{files} 个文件',
      createTitle: '创建项目',
      namePlaceholder: '项目名称',
      instructionsPlaceholder: '项目指令',
      createProject: '创建项目',
      empty: '暂无项目上下文',
      emptyHelp: '先创建一个项目，再导入仓库、文件夹或手动笔记；启用后相关内容会随提问自动加入上下文。',
      deleteProject: '删除',
      deleteConfirm: '确定删除项目“{name}”吗？',
      activateProject: '激活 {name}',
      fileCount: '{count} 个文件',
      activeProject: '当前项目：{name}',
      githubPlaceholder: 'https://github.com/owner/repo',
      githubTokenPlaceholder: 'GitHub Token（可选，不会保存）',
      githubTokenHelp: '私有仓库或遇到 403/rate limit 时填写 fine-grained token；只需要仓库只读权限，DeepSeek++ 不会保存或同步。',
      importGithub: '导入 GitHub 仓库',
      importingGithub: '导入中...',
      permissionError: '需要 GitHub API 和 raw 内容访问权限才能导入项目上下文',
      importComplete: '已导入 {count} 个文件。',
      importCompleteWithWarnings: '已导入 {count} 个文件，{warnings} 条警告。',
      manualPathPlaceholder: '手动文件路径',
      manualContentPlaceholder: '手动文件内容',
      addManualFile: '添加手动文件',
    ...
    whatsNew: {
      ...
      interactiveTools: '沙箱确认、Skill 草稿、记忆导入、语音、保存片段和提示词控制已可用。',
      productSurfaces: '保存项导出、历史组织、代码下载和开发者 playground 已可用。',
    },
    memoryPage: {
      title: '记忆',
    ...
    settings: {
      ...
      syncCounts: '记忆 {memories} 条，Skill {skills} 个，预设 {presets} 个，项目 {projects} 个，项目文件 {projectFiles} 个，保存项 {savedItems} 条',
    ...
    developerSettings: {
      title: '开发者选项',
      developerMode: '开发者模式',
      developerModeDescription: '显示用于测试 DeepSeek++ 行为的本地高级工具',
      apiPlayground: 'API playground',
      apiPlaygroundDescription: '使用本地保存的 API Key 手动测试官方 API 调用',
      cssPolicy: '任意自定义 CSS 继续禁用，只允许有边界的第一方主题控制。',
    },
    developerPage: {
      title: 'API playground',
      description: '运行一次手动 DeepSeek API 测试调用。API Key 不会展示、同步或导出。',
      disabled: '开发者模式未启用。请在设置中打开后再使用本地测试工具。',
      playgroundDisabled: 'API playground 已在开发者选项中关闭。',
      promptPlaceholder: '用于单次 API 测试的提示词',
      run: '运行 API 调用',
      running: '运行中...',
      runFailed: 'API playground 运行失败',
    },
    voice: {
      title: '语音',
      input: '语音输入',
  ...
  tool: {
    ...
    artifact: {
      createTitle: '创建可下载文件',
      createDescription: '为用户创建可下载文件。适合生成脚本、Markdown、HTML、JSON 或其他文本产物。',
    ...
    sandbox: {
      runTitle: '请求沙箱运行',
      runDescription: '准备 JavaScript、TypeScript 或 Python 代码供用户确认后执行。用户点击运行前不会执行代码。',
      languageDescription: '运行语言：javascript、typescript 或 python',
      codeDescription: '用户确认后要执行的代码',
      inputDescription: '可选输入字符串，代码中可读取',
      timeoutDescription: '超时时间，单位毫秒，默认 5000',
      approvalRequired: '需要确认',
      approvalDetail: '代码已准备好，但只有用户确认后才会运行。',
      invalidRequest: '沙箱请求无效',
      unsupportedApprovedLanguage: '不支持的确认运行语言',
      pythonUnavailable: 'Python 沙箱不可用',
      pythonUnavailableDetail: '请先通过 Shell Native Host 启用 python_exec，再确认 Python 沙箱运行。',
    },
    skillCreator: {
      title: '起草 Skill',
